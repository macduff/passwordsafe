...
ON_COMMAND(ID_TOOLBUTTON_SENDEMAIL, OnSendEmail)
...
DboxMain::OnInitMenu(CMenu* pMenu)
{
  ...
  if (bItemSelected) {
    ...
    if (ci->IsEmailEmpty()) {
      pMenu->EnableMenuItem(ID_MENUITEM_SENDEMAIL, MF_GRAYED);
    } else {
      pMenu->EnableMenuItem(ID_MENUITEM_SENDEMAIL, MF_ENABLED);
    }
...
DboxMain::UpdateMenuAndToolBar(const bool bOpen)
{
  ...
  xfilesubmenu = xmainmenu->GetSubMenu(pos);
    ...
    int condOpen[] = {ID_TOOLBUTTON_COPYPASSWORD, ID_TOOLBUTTON_COPYUSERNAME,
                      ...
                      ID_TOOLBUTTON_AUTOTYPE, ID_TOOLBUTTON_BROWSEURL, ID_TOOLBUTTON_SENDEMAIL,
                      ID_TOOLBUTTON_EDIT};

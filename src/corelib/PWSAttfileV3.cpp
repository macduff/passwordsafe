...
int PWSAttfileV3::WriteAttmntRecord(const ATRecord &atr)
{
  ASSERT(m_curversion == V30);
  ...
  WriteCBC(ATTMT_USIZE, (unsigned char *)&atr.uncsize, sizeof(atr.uncsize));
  WriteCBC(ATTMT_CSIZE, (unsigned char *)&atr.cmpsize, sizeof(atr.cmpsize));

  WriteCBC(ATTMT_CRC, (unsigned char *)&atr.CRC, sizeof(atr.CRC));
  WriteCBC(ATTMT_DIGEST, (unsigned char *)&atr.digest, sizeof(atr.digest));
  ...
  WriteCBC(ATTMT_DATA, atr.pData, atr.cmpsize);
...
int PWSAttfileV3::ReadAttmntRecord(ATRecord &atr, const uuid_array_t &attmt_uuid,
                                   const bool bSkipAll, const bool bSkipUnless)
{
  ASSERT(m_curversion == V30);
  ...
  time_t t;
  int i32;
  bool go(true), bGotUUID(false);
  unsigned char pSkipTypes[256] = {0};
  pSkipTypes[ATTMT_DATA] = 0x01;  // If we skip a field - this is the one

  do {
    unsigned char *utf8 = NULL;
    int utf8Len = 0;
    bool bSkip = bSkipAll || (bGotUUID && !bSkipUnless && 
                    memcmp(atr.attmt_uuid, attmt_uuid, sizeof(attmt_uuid)) == 0);
      ...
      switch (type) {
        case ATTMT_UUID:
          if (utf8Len != sizeof(uuid_array_t)) {
            go = false; status = BAD_ATTACHMENT; break;
          }
        ...
        case ATTMT_DIGEST:
          ...
          memcpy(atr.digest, utf8, SHA1::HASHLEN);
          break;
        case ATTMT_CTIME:
        ...
        case ATTMT_DATA:
          ...
          atr.cmpsize = utf8Len;
          if (bSkipAll || (bGotUUID && !bSkipUnless && 
                       memcmp(atr.attmt_uuid, attmt_uuid, sizeof(attmt_uuid)) != 0)) {
            atr.pData = NULL;
          } else {
            atr.pData = new unsigned char[utf8Len];
            memcpy(atr.pData, utf8, utf8Len);
          }
          break;
        case ATTMT_END:
          break;

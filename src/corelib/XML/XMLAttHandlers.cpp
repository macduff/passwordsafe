...
void XMLAttHandlers::ProcessEndElement(const int icurrent_element)
{
  ...
  switch (icurrent_element) {
    ...
    case XLA_ATTACHMENT_UUID:
      ConvertFromHex(m_strElemContent, sizeof(uuid_array_t), cur_entry->atr.attmt_uuid);
      break;
    case XLA_ENTRY_UUID:
      ConvertFromHex(m_strElemContent, sizeof(uuid_array_t), cur_entry->atr.entry_uuid);
    ...
    case XLA_CRC:
      wchar_t *p;
      cur_entry->atr.CRC = wcstol(m_strElemContent.c_str(), &p, 16);
      break;
    case XLA_ODIGEST:
      ConvertFromHex(m_strElemContent, sizeof(cur_entry->atr.digest), cur_entry->atr.digest);
      break;
    case XLA_CDIGEST:
       ConvertFromHex(m_strElemContent, sizeof(cur_entry->cdigest), cur_entry->cdigest);
      break;
    case XLA_CTIME:
    ...
    case XLA_DATA:
      if (!m_bIgnoreThisAttachment) {
        ...
        cur_entry->atr.pData = new unsigned char[out_len];
        PWSUtil::Base64Decode(m_strElemContent, cur_entry->atr.pData, out_len);
        ASSERT(cur_entry->atr.cmpsize == out_len);
        if (cur_entry->atr.cmpsize != out_len) {
          delete [] cur_entry->atr.pData;
          cur_entry->atr.pData = NULL;
        ...
        SHA1 context;
        context.Update(cur_entry->atr.pData, cur_entry->atr.cmpsize);
        context.Final(cdigest);
        if (memcmp(cdigest, cur_entry->cdigest, SHA1::HASHLEN) != 0) {
          delete [] cur_entry->atr.pData;
          cur_entry->atr.pData = NULL;
...
void XMLAttHandlers::AddAttachments()
{
  vatt_entries::iterator att_iter;

  for (att_iter = m_ventries.begin(); att_iter != m_ventries.end(); att_iter++) {
    att_entry *cur_entry = *att_iter;

    if (cur_entry->err_flags != 0 || cur_entry->atr.pData == NULL)
      continue;
    ...
    if (!VerifyImportDateTimeString(cur_entry->sx_dtime.c_str(), cur_entry->atr.dtime))
      cur_entry->atr.dtime = (time_t)0;
    ...
    m_pXMLcore->AddAttachment(cur_entry->atr);
  }
}

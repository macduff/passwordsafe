...
static EAttHandlers *pFileHandler(NULL);
...
EAttXMLProcessor::EAttXMLProcessor(PWScore *pcore,
    ...
    m_pmulticmds(p_multicmds), m_prpt(prpt), m_delimiter(TCHAR('^'))
{
  ...
  pFileHandler = new EAttHandlers;
}

EAttXMLProcessor::~EAttXMLProcessor()
{
  if (pFileHandler) {
    delete pFileHandler;
    pFileHandler = NULL;
...
// ---------------------------------------------------------------------------
bool EAttXMLProcessor::Process(const bool &bvalidation,

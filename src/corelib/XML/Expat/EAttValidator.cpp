...
/*
* This routine validates Attachment XML when using the Expat library V2.0.1
...
bool EAttValidator::startElement(stringT & strStartElement)
{
  m_iErrorCode = 0;
  if (strStartElement == _T("passwordsafe_attachments")) {
...
bool EAttValidator::endElement(stringT &endElement, StringX &strValue, int &datatype)
{
...
bool EAttValidator::VerifyXMLDataType(const StringX &strElemContent, const int &datatype)
{
...
bool EAttValidator::VerifyXMLDate(const StringX &strValue)
{
  // yyyy-mm-dd
...
bool EAttValidator::VerifyXMLTime(const StringX &strValue)
{
  // hh:mm:ss
  const int ndigits = 6;
...
StringX EAttValidator::Trim(const StringX &s, const TCHAR *set)
{

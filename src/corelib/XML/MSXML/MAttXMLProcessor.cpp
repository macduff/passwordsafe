...
// MAttXMLProcessor.cpp : implementation file
//
...
MAttXMLProcessor::MAttXMLProcessor(PWScore *pcore,
                                   MultiCommands *p_multicmds,
                                   CReport *prpt)
  : m_pXMLcore(pcore), m_MSXML_Version(60),
  m_pmulticmds(p_multicmds), m_prpt(prpt)
{
}

MAttXMLProcessor::~MAttXMLProcessor()
{
}

// ---------------------------------------------------------------------------
bool MAttXMLProcessor::Process(const bool &bvalidation,
                               const stringT &strXMLFileName, const stringT &strXSDFileName)
{
  ...
  MAttSAX2ContentHandler* pCH = new MAttSAX2ContentHandler();
  pCH->SetVariables(m_bValidation ? NULL : m_pXMLcore, m_bValidation, 
                    m_pmulticmds, m_prpt);

  //  Create ErrorHandlerImpl object
  MAttSAX2ErrorHandler* pEH = new MAttSAX2ErrorHandler();
  ...
  if (!FAILED(hr)) {  // Create SchemaCache
    ...
    if(!FAILED(hr)) {  // Check for parsing errors
      if(pEH->bErrorsFound == TRUE) {
        ...
        if (m_bValidation) {
          m_numAttachmentsValidated = pCH->m_numAttachments;
        } else {
          // Now add entries
          pCH->AddAttachments();

          // Get numbers (may have been modified by AddEntries
          m_numAttachmentsImported = pCH->m_numAttachments;
          m_numAttachmentsSkipped = pCH->getNumSkipped();

...
// MAttSAX2Handlers.cpp : implementation file
//
...
//  -----------------------------------------------------------------------
//  MAttSAX2ErrorHandler Methods
//  -----------------------------------------------------------------------
MAttSAX2ErrorHandler::MAttSAX2ErrorHandler()
...
long __stdcall MAttSAX2ErrorHandler::QueryInterface(const struct _GUID &riid,void ** ppvObject)
{
  *ppvObject = NULL;
...
unsigned long __stdcall MAttSAX2ErrorHandler::AddRef()
{
  return ++m_refCnt; // NOT thread-safe
}

unsigned long __stdcall MAttSAX2ErrorHandler::Release()
{
...
//  -----------------------------------------------------------------------
//  MAttSAX2ContentHandler Methods
//  -----------------------------------------------------------------------
MAttSAX2ContentHandler::MAttSAX2ContentHandler()
{
  m_refCnt = 0;
  m_pValidator = new MAttValidator;
}
...
long __stdcall MAttSAX2ContentHandler::QueryInterface(const struct _GUID &riid,void ** ppvObject)
{
  *ppvObject = NULL;
  if (riid == IID_IUnknown || riid == __uuidof(ISAXContentHandler)) {
...
unsigned long __stdcall MAttSAX2ContentHandler::AddRef()
{
  return ++m_refCnt; // NOT thread-safe
}

unsigned long __stdcall MAttSAX2ContentHandler::Release()
{
  --m_refCnt; // NOT thread-safe
  if (m_refCnt == 0) {
    delete this;
...
//  -----------------------------------------------------------------------
HRESULT STDMETHODCALLTYPE  MAttSAX2ContentHandler::startDocument()
{
  m_strXMLErrors = _T("");
...
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::putDocumentLocator(
                                 struct ISAXLocator * pLocator)
{
  return S_OK;
}

TCHAR * AttProcessAttributes(
...
//  ---------------------------------------------------------------------------
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::startElement(
  ...
  /* [in] */ ISAXAttributes __RPC_FAR *pAttributes)
{
  ...
  if (!XMLAttHandlers::ProcessStartElement(icurrent_element))
    return S_OK;
...
//  ---------------------------------------------------------------------------
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::characters(
...
//  -----------------------------------------------------------------------
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::endElement(
  ...
  int cchQName)
{
  ...
  if (m_bValidation) {
    if (wcscmp(szCurElement, L"attachment") == 0)
      m_numAttachments++;
    return S_OK;
  }
...
//  ---------------------------------------------------------------------------
HRESULT STDMETHODCALLTYPE  MAttSAX2ContentHandler::endDocument()
{
  return S_OK;
}

HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::startPrefixMapping(
...
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::endPrefixMapping(
  const wchar_t * pwchPrefix,
  int cchPrefix)
{
  return S_OK;
...
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::ignorableWhitespace(
...
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::processingInstruction(
...
HRESULT STDMETHODCALLTYPE MAttSAX2ContentHandler::skippedEntity(

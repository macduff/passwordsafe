                           ...
                           const OrderedItemList *il)
{
  ...
  if (bAdvanced) {
    ...
    if (il != NULL)
      return il->empty() ? PWSRC::FAILURE : PWSRC::SUCCESS;
    else
      return m_pwlist.empty() ? PWSRC::FAILURE : PWSRC::SUCCESS;
  }
                                    ...
                                    UINT &uiReasonCode, CReport &rpt, Command *&pcommand)
{
  ...
  return ((numSkipped + numRenamed)) == 0 ? PWSRC::SUCCESS : PWSRC::OK_WITH_ERRORS;
}

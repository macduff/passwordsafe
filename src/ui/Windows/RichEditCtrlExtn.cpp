  ...
  *pResult = 0;
  ...
  if (pNotifyStruct->code == EN_LINK) {
    if (pENLink->msg == WM_LBUTTONDOWN || pENLink->msg == WM_LBUTTONDBLCLK) {
      ...
      vector<ALink>::const_iterator found;
      found = find_if(m_vALink.begin(), m_vALink.end(), StartEndMatch);
...
void CRichEditCtrlExtn::SetWindowText(LPCWSTR lpszString)
{
  ...
  if (iError != 0) {
    ...
    // Now apply formating
    if (!m_vFormat.empty()) {
      sort(m_vFormat.begin(), m_vFormat.end(), iStartCompare);
      vector<st_format>::const_iterator format_iter;
    ...
    if (!m_vALink.empty()) {
      ...
      vector<ALink>::const_iterator ALink_iter;
      memset((void *)&cf2, 0x00, sizeof(cf2));
      cf2.cbSize = sizeof(cf2);
      cf2.dwMask = CFM_LINK;
...
CString CRichEditCtrlExtn::GetTextFormatting(CString csHTML, int &iError)
{
  ...
  bitset<3> bsFontChange;  // facename, size, color

  vector<CString> vLastFacenames;
  vector<COLORREF> vLastColours;
  vector<int> vLastSizes;
  vector<int> vFontChangeStart;
  vector<bitset<3>> vFontChange;
  stack<st_format> format_stack;
...
vnext:
  ...
  while (csToken != "" && curPos != -1) {
    ...
    if (bHTMLTag) {
      ...
      if (csHTMLTag == L"/font") {
        bitset<3> &bsLastFont = vFontChange.back();
...
COLORREF CRichEditCtrlExtn::ConvertColourToColorRef(CString &csValue)
{
  ...
  if (csValue.Left(1) == L"#") {
    ...
    int icolour;
#if _MSC_VER >= 1400
    _stscanf_s(csValue.Mid(1), L"%06x", &icolour);
#else
    _stscanf(csValue.Mid(1), L"%06x", &retval);
#endif

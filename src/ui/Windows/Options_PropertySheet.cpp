...
void COptions_PropertySheet::SetupInitialValues()
{
  ...
  if (m_OPTMD.uicaller == IDS_OPTIONS) {
    // Security Data
    m_OPTMD.EraseProgram =
        prefs->GetPref(PWSprefs::EraseProgram).c_str();
    m_OPTMD.ErasePgmCmdLineParms = 
        prefs->GetPref(PWSprefs::ErasePgmCmdLineParms).c_str();
...
void COptions_PropertySheet::UpdateCopyPreferences()
{
  ...
  prefs->SetPref(PWSprefs::EraseProgram, 
                 LPCWSTR(m_OPTMD.EraseProgram), true);
  prefs->SetPref(PWSprefs::ErasePgmCmdLineParms,
                 LPCWSTR(m_OPTMD.ErasePgmCmdLineParms), true);

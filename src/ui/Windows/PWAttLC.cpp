...
void CPWAttLC::Init(const LCType lct, CWnd *pWnd)
{
  m_lct = lct;
  m_pParent = GetParent();
  m_pWnd = pWnd;
...
void CPWAttLC::OnRButtonDown(UINT nFlags, CPoint point)
{
  ...
  if ((iItem >= 0)    && (iItem < m_numattachments) &&
    ...
    DoubleToLocalizedString(::GetThreadLocale(), dblVar, 0,
    ...
    if (m_lct != NEW) {
      ...
      wostringstream os;
      for (size_t i = 0; i < sizeof(pATR->digest); i++) {
        os << setw(2) << setfill(wchar_t('0')) << hex << int(pATR->digest[i]);
        if ((i % 4) == 3)
          os << L" ";
      }
      st_prop.digest = os.str().c_str();

      if (pATR->uncsize != 0) {
        dblVar = 100.0 - pATR->cmpsize * 100.0 / pATR->uncsize;
        DoubleToLocalizedString(::GetThreadLocale(), dblVar, 1,
                                wcbuffer, sizeof(wcbuffer) / sizeof(wchar_t));

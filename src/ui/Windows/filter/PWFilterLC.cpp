...
void CPWFilterLC::Init(CWnd * pParent, st_filters *pfilters, const int &filtertype)
{
  ...
  for (Flt_iter = m_pvfdata->begin(); Flt_iter != m_pvfdata->end(); Flt_iter++) {
    ...
    if (st_fldata.ftype != FT_INVALID) {
      ...
      vcbxChanged[i] = false;
      vector<st_Fcbxdata>::iterator Fcbxdata_iter;
      Fcbxdata_iter = find_if(vFcbx_data.begin(), vFcbx_data.end(), 
...
bool CPWFilterLC::SetField(const int iItem)
{
  ...
  if (ft != FT_INVALID) {
    vector<st_Fcbxdata>::iterator Fcbxdata_iter;
    Fcbxdata_iter = find_if(vWCFcbx_data.begin(), vWCFcbx_data.end(), equal_ftype(ft));
...
void CPWFilterLC::SetLogic(const int iItem)
{
  ...
  if (lt != LC_INVALID) {
    vector<st_Lcbxdata>::iterator Lcbxdata_iter;
    Lcbxdata_iter = find_if(vLcbx_data.begin(), vLcbx_data.end(), equal_ltype(lt));
...
void CPWFilterLC::DrawComboBox(const int iSubItem, const int index)
{
  ...
  if (nindex >= n || nindex < 0)
    ...
    if (iSubItem == FLC_FLD_COMBOBOX) {
      vector<st_Fcbxdata>::iterator Fcbxdata_iter;
      Fcbxdata_iter = find_if(vWCFcbx_data.begin(), vWCFcbx_data.end(),

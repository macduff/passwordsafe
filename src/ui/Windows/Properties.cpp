...
BOOL CProperties::OnInitDialog()
{
  ...
  tmp1.Format(_T("%d"), m_pdbp->num_att);
  if (m_pdbp->num_att == 0) {
    tmp2 = tmp3 = tmp4 = L"n/a";
  } else {
    double dblVar;
    wchar_t wcbuffer[40];
    dblVar = (m_pdbp->totalunc + 1023.0) / 1024.0;
    ...
    dblVar = (m_pdbp->totalcmp + 1023.0) / 1024.0;;
    PWSNumUtil::DoubleToLocalizedString(::GetThreadLocale(), dblVar, 1,
                                        wcbuffer, sizeof(wcbuffer) / sizeof(wchar_t));
    wcscat_s(wcbuffer, sizeof(wcbuffer) / sizeof(wchar_t), L" KB");
    tmp3 = wcbuffer;

    dblVar = (m_pdbp->largestunc + 1023.0) / 1024.0;;
    PWSNumUtil::DoubleToLocalizedString(::GetThreadLocale(), dblVar, 1,
                                        wcbuffer, sizeof(wcbuffer) / sizeof(wchar_t));
    wcscat_s(wcbuffer, sizeof(wcbuffer) / sizeof(wchar_t), L" KB");
    tmp4 = wcbuffer;

    dblVar = (m_pdbp->largestcmp + 1023.0) / 1024.0;;
    PWSNumUtil::DoubleToLocalizedString(::GetThreadLocale(), dblVar, 1,
                                        wcbuffer, sizeof(wcbuffer) / sizeof(wchar_t));
    wcscat_s(wcbuffer, sizeof(wcbuffer) / sizeof(wchar_t), L" KB");
    tmp4 += L" / ";

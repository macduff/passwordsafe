    ...
    int cchQName)
{
    ...
    if (!cur_entry->uhrxl.empty()) {
      UnknownHeaderFieldList::const_iterator vi_IterUXRFE;
      for (vi_IterUXRFE = cur_entry->uhrxl.begin();
           vi_IterUXRFE != cur_entry->uhrxl.end();
           vi_IterUXRFE++) {
        UnknownHeaderFieldEntry unkrfe = *vi_IterUXRFE;
#ifdef _DEBUG
  ...
  if (_tcscmp(szCurElement, _T("field")) == 0) {
    ...
    UnknownHeaderFieldEntry ukxfe;
    ukxfe.uc_Type = m_ctype;
    ukxfe.st_Length = m_fieldlen;
    ukxfe.uc_pUField = new unsigned char[m_fieldlen + sizeof(TCHAR)];
    memset(ukxfe.uc_pUField, 0x00, m_fieldlen + sizeof(TCHAR));
    memcpy(ukxfe.uc_pUField, m_pfield, m_fieldlen);
    if (m_bheader) {

...
void CTVTreeCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
  ...
  cs_text.Format("%s%02x%08x", gbl_classname, FROMTREE, lBufLen);

  CMemFile mf;
  mf.Write((LPCSTR)cs_text, sizeof(gbl_classname) - 1 + 10);
...
bool CTVTreeCtrl::CollectData(BYTE * &out_buffer, long &outLen)
{
  ...
  poutDDmemfile = new CSMemFile;
  CArchive ar_out (poutDDmemfile, CArchive::store);
  out_oblist.Serialize(ar_out);
  ar_out.Flush();
  ar_out.Close();

  DWORD dw_outmflen = (DWORD)poutDDmemfile->GetLength();
  unsigned char * outddmemfile_buffer = (unsigned char *)poutDDmemfile->Detach();
                                      ...
                                      unsigned char * &out_buffer, long &outLen)
{
  ...
  passwd.Format("%s%04x", m_sending_classname, gbl_tcddCPFID);
                                     ...
                                     BYTE * &out_buffer, long &outLen)
{
  ...
  passwd.Format("%s%04x", gbl_classname, gbl_tcddCPFID);
